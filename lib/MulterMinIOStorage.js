...
MinIOStorage.prototype._handleFile = function (req, file, cb) {
  collect(this, req, file, function (err, opts) {
    ...
    if (opts.shouldCreateThumbnail) {
      this.minioClient.putObject(opts.bucket, (0, _parseFilekey.parseFileKey)(opts.key, 'thumb'), thumbStream, {
    ...
    if (opts.shouldCreateFeatured) {
      this.minioClient.putObject(opts.bucket, (0, _parseFilekey.parseFileKey)(opts.key, 'featured'), featuredStream, {
        'Content-Type': opts.contentType
      }, function (err, etag) {
        if (err) cb(err);else {
          cb(null, {
    ...
    this.minioClient.putObject(opts.bucket, opts.key, file.stream, file.size, {
      'Content-Type': opts.contentType
    }, function (err, etag) {
      if (err) cb(err);else {
        cb(null, {
